...
但当构造函数参数过多时，会导致构造函数过长，影响可读性和易用性。

**改进：**

当参数增加时，可在类中增加set方法设置选填参数，构造函数设置必填项。
...
建造者模式能够将对象的创建与表示分离，即将校验逻辑放置Builder类中，通过建造者的set()方法设置建造者的变量值，在使用build()方法创建对象之前，做集中的校验，校验之后才会生成对象。且所创建类的构造方法为私有，且没有提供set()方法，这样创建出来的也就是不可变对象。
...
若不关心对象是否处于无效状态或可变的，直接使用set也行，毕竟Builder中的成员变量属于重复性代码。
