...
​		当使用equals()方法比较时，若类没有重写equals()方法，则相当于通过"=="来比较这两个对象，默认使用Object类的equals()方法。若重写了equals()方法，则一般比较对象的内容是否相等。<!--String类中重写了equals()方法，比较值是否相等，当创建String类型的对象时，JVM会在常量池中查找是否有存在的相同的值，若有，则将其赋值给当前引用，没有则在常量池中重新创建一个String对象-->
